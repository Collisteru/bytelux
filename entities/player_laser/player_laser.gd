...
func fire_laser(laser_position, player_position):
	...
	if (x_diff >= 0 and y_diff < 0):
		angle = atan(y_diff / x_diff) + 2 * PI
	# Quadrant II (top-left, neg x_diff, neg y_diff
	elif (x_diff < 0 and y_diff < 0):
		print("Quad 2!")
		angle = atan(y_diff / x_diff) + PI
	# Quadrant III (bottom-left, neg x_diff, pos y_diff)
	elif (x_diff < 0 and y_diff >= 0):
		angle = PI + atan(y_diff / x_diff)
	# Quadrant IV (bottom-right, pos x_diff, pos y_diff)
	elif (x_diff >= 0 and y_diff >= 0):
